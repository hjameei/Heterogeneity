...
for i=1:length(code_self_v2)
    ...
    subs_self{i}=subID(ind_sub);
    age_diag_self{i}=age_tmp;
    fprintf('%s: n=%d\n\n',dx_labels{i},length(subs_self{i}));
end
...
for i=1:length(code_icd9) % loop over diseases
    ...
    subs_icd9{i}=subID(ind_sub);
    age_diag_icd9{i}=age_tmp;
    date_diag_icd9{i}=date_icd9_tmp;
    fprintf('%s: n=%d\n\n',dx_labels{i},length(subs_icd9{i}));   
end
...
for i=1:length(code_icd10)
    ...
    subs_icd10{i}=subID(ind_sub);
    age_diag_icd10{i}=age_tmp;
    date_diag_icd10{i}=date_icd10_tmp;
    fprintf('%s: n=%d\n\n',dx_labels{i},length(subs_icd10{i}));
end
...
%subjects without any icd9 diagnoses
ind_health_icd9=find(all(isnan(num_icd9),2)); %subjects with all nans

%subjects without any icd10 and self diagnoses
ind_health_icd10=[];
for i=1:size(txt_icd10,1)
    tmp=cell2mat(txt_icd10(i,:));
    if isempty(tmp)
        ind_health_icd10=[ind_health_icd10,i];
    end
end
...
% filename = [Out_private, 'subID_icd_self.mat'];
 save(filename,'dx_labels','dx_organ','dx_system',...
    'subID_completed_icd9','subID_completed_icd10','subID_completed_self','subID_completed_mhq',...
    'subID_PNA_or_missing_icd9','subID_PNA_or_missing_icd10','subID_PNA_or_missing_self','subID_PNA_or_missing_mhq,',...
    'subID_healthy_icd9','subID_healthy_icd10','subID_healthy_self','subID_healthy_mhq','subID_healthy_all',...
    'subID_dx_icd9','subID_dx_icd10','subID_dx_self','subID_dx_mhq','subID_dx_all',...
    'date_completed_icd9','date_completed_icd10','date_completed_self','date_completed_mhq');
